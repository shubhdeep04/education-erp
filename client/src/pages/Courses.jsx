...
export default function Courses() {
  ...
  const [search,      setSearch]      = useState("")
  const [view,        setView]        = useState("grid")
  const [enrolledIds, setEnrolledIds] = useState([])  // ✅ enrolled course IDs
  const [loadingId,   setLoadingId]   = useState(null) // ✅ loading state per button
  ...
  // ✅ Already enrolled courses fetch karo
  useEffect(() => {
    const fetchEnrollments = async () => {
      try {
        const res  = await fetch("http://localhost:5000/api/enrollment/mine", { credentials: "include" })
        const data = await res.json()
        if (data.success) {
          setEnrolledIds(data.enrollments.map(e => e.courseId))
        }
      } catch {
        // Login nahi hai — koi baat nahi, enrolledIds empty rahega
      }
    }
    fetchEnrollments()
  }, [])
  ...
  const handleEnroll = async (course) => {
    try {
      ...
      const res  = await fetch("http://localhost:5000/api/enrollment/enroll", {
        method: "POST",
  ...
  // ✅ Enroll button — enrolled hai ya nahi uske hisaab se dikhao
  const EnrollBtn = ({ course, className = "course-link", style = {} }) => {
    const isEnrolled = enrolledIds.includes(course.id)
    const isLoading  = loadingId === course.id
    if (isEnrolled) {
      return (
        <span style={{ fontSize:13, fontWeight:700, color:"#4ade80", ...style }}>
          ✅ Enrolled
        </span>
      )
    }
    return (
      <button
        ...
        disabled={isLoading}
        className={className}
        style={{ cursor: isLoading ? "not-allowed" : "pointer", opacity: isLoading ? 0.7 : 1, border:"none", ...style }}
      >
                ...
                <div key={c.id||i} className={`card course-card fade-up delay-${(i%3)+1}`}>
                  ...
                  <div className="course-body">
                    ...
                    <div className="course-footer">
                      <span className="course-price">{getPrice(c)}</span>
                      {/* ✅ Enroll button */}
                      <EnrollBtn course={c} className="course-link" />
                    </div>
                  </div>
                </div>
              ))}
                  ...
                  <div style={{ padding:28 }}>
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:16 }}>
                        <span style={{ fontFamily:"'Playfair Display',serif", fontSize:22, color:"var(--gold)" }}>{getPrice(c)}</span>
                        {/* ✅ Enroll button */}
                        <EnrollBtn course={c} className="btn-primary" style={{ fontSize:13, padding:"10px 22px" }} />
                      </div>
                    </div>
                  </div>
                </div>
